...
- `TextureCache` — registre de textures chargées à la demande par **nom logique**, avec validation à l'entrée, `invalidate`/`invalidateAll` (rechargement à chaud, `LOT-43`) et mémorisation des échecs (`LOT-40`, `EX-REN-043`). Ne réimplémente ni le décodage ni l'upload : il compose `AssetPaths` et `TextureLoader`. La mémoïsation/invalidation elle-même est factorisée dans `CacheRegistry` (générique, sans Direct3D), testable sans GPU.
