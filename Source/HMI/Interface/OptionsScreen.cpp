...
ScreenTransition OptionsScreen::update(const InputState& input, float /*fixedDelta*/) {
    ...
    const std::optional<OptionsAction> action = _model.update(input);
    if (action) {
        switch (*action) {
...
// Dessine le titre, les options, l'état de la manette et le bouton de langue.
void OptionsScreen::render(RenderContext& context) {
    ...
    context.font.drawText(context.spriteBatch, _localization.text("options.titre"),
                          ...
                          TITLE_COLOR);
    for (int index = 0; index < OptionsModel::OPTION_COUNT; ++index) {
    ...
    // Ligne d'état, sous les options : information seule, non navigable (EX-CTRL-002).
    const std::string gamepadStatus = _localization.text(
        _gamepadConnected ? "options.manette_connectee" : "options.manette_absente");
    const float statusY = MenuModel::optionTop(OptionsModel::OPTION_COUNT) + 32.0f;
