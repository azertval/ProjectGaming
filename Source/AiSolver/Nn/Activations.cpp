    ...
    return autodiff::unaryOp(
        ...
        [](const Tensor<float>& value) {
            return aisolver::detail::elementwiseUnary(
                value, [](float x) { return 1.0f / (1.0f + std::exp(-x)); });
        },
...
autodiff::NodePtr softmax(const autodiff::NodePtr& a) {
    return autodiff::unaryOp(
        ...
        [](const Tensor<float>& value) {
            ...
            const Tensor<float> shifted = aisolver::detail::elementwiseUnary(
                value, [maxValue](float x) { return x - maxValue; });
        ...
        [](const Tensor<float>& outputValue, const Tensor<float>& outputGrad,
           const Tensor<float>&) {
            ...
            return aisolver::detail::elementwise(
                outputValue, outputGrad,
                [dot](float softmaxI, float gradI) { return softmaxI * (gradI - dot); });
        });
}
