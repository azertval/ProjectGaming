...
| Pente/arrondi de plafond | **Solide** (contrairement aux variantes de sol ci-dessus) : miroir vertical de la même silhouette (matière pleine en haut de la case), bloque comme un mur incliné/courbe par le dessus. Jamais suivi par le personnage — silhouette purement visuelle. |
...
- \anchor EX-GP-006 **EX-GP-006** — Une tuile de **pente/arrondi de plafond** (`SlopeDownRight`/`SlopeDownLeft`/`RoundedDownRight`/`RoundedDownLeft`) doit exister comme variante **miroir vertical** des pentes/arrondis de sol (`EX-GP-003`/`EX-GP-004`) : matière pleine en **haut** de la case plutôt qu'en bas. Contrairement à ses équivalents de sol, elle est **solide** (`core::isSolid`) et bloque le personnage comme un mur incliné/courbe par le dessus — elle n'est **jamais suivie** par `core::resolveSlopeFollow` (pas de marche le long d'un plafond) : sa silhouette inclinée/courbe n'a de valeur que **visuelle** (@ref guide-rendu), la collision réelle reste une case pleine.
