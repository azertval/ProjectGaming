...
- **Widget de vignettes partagé**, utilisé par les trois sections existantes du panneau « Textures »
  ...
  ni son modèle de données : `hmi::TextureLoader::decodeImageFile` → `QPixmap`, indépendant du
...
existantes). Réutilise `hmi::TextureLoader`/`hmi::TextureCache`. Prépare [LOT-48](@ref lot-48)
